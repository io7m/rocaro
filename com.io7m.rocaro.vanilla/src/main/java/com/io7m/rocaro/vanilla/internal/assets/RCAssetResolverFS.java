  ...
  implements RCAssetResolverType
{
    ...
    throws RCAssetException
  {
    ...
    final var packageDirectory =
      this.baseDirectory.resolve(identifier.packageName().value());

    final var pathSegments =
      identifier.path().elements();

    var filePath = packageDirectory;
    for (final var element : pathSegments) {
      filePath = filePath.resolve(element);
    }

    if (!filePath.startsWith(this.baseDirectory)) {
  ...
  {
    return "Filesystem asset resolver service.";
  }
}
