...
/**
 * The image depth node.
 */

public enum RCImageNodeDepth
  implements RCGNodeDescriptionFactoryType<
  RCImageParametersDepth,
  RCImageNodeType<RCImageParametersDepth>,
  RCImageNodeDescriptionType<RCImageParametersDepth, RCImageDepthType>>
{
  ...
  @Override
  public RCImageNodeDescriptionType<RCImageParametersDepth, RCImageDepthType> createDescription(
    ...
    final RCGNodeName name)
  {
    final var output =
      new RCGPortProducer<>(
        name,
        SAMPLE_PORT_NAME,
        new RCImageConstraintDepth(Optional.of(parameters.size()))
      );

    final var ports = new HashMap<RCGPortName, RCGPortType<?>>();
    ports.put(output.name(), output);
    return new RCImageNodeDepthDescription(parameters, name, ports);
  }
