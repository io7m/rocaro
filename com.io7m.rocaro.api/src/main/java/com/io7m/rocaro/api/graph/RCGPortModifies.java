...
/**
 * A port that modifies its input and passes the modified value to its output.
 *
 ...
 * @param requiresImageLayout The required image layout, for image-typed resources
 * @param ensuresImageLayout  The ensured image layout, for image-typed resources
 */

public record RCGPortModifies(
  ...
  implements RCGPortProducerType, RCGPortConsumerType
{
  /**
   * A port that modifies its input and passes the modified value to its output.
   *
  ...
  public RCGPortModifies
  {
    ...
    if (requiresImageLayout.isPresent()) {
      Preconditions.checkPreconditionV(
        type,
        RCGResourceTypes.isImage(type),
        "Ports can only specify image layout requirements for image resources."
      );
    }
