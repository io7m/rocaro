...
import com.io7m.rocaro.vanilla.internal.graph.RCGPassPrimitiveResourcesTrack;
import com.io7m.rocaro.vanilla.internal.graph.RCGPassResourcesTrack;
import com.io7m.rocaro.vanilla.internal.graph.RCGPassTopological;
  ...
  implements RCGGraphPassType
{
  ...
  private HashMap<RCGPortPrimitiveType, RCGResourceVariable<? extends RCResourceSchematicPrimitiveType>> portResources;
  ...
  public RCGPassSyncPrimitive()
  {
    super(Set.of(
      ...
      RCGPassPrimitiveResourcesTrack.class,
      RCGPassResourcesTrack.class,
    ...
    final RCGOperationType op)
  {
    ...
    for (final var port : op.ports().values()) {
      final var primitives =
        this.portToPrimitives.get(port);

      for (final var primitive : primitives.values()) {
        final var resource =
          this.portResources.get(primitive);
    ...
    for (final var port : op.ports().values()) {
      ...
      for (final var primitivePort : primitives.values()) {
        final var resource =
          this.portResources.get(primitivePort);
        final var imageLayoutTransition =
          builder.portImageLayouts().get(primitivePort);

        this.processPort(opCmd, resource, imageLayoutTransition, primitivePort);
      }
    }
  }
